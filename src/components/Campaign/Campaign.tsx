...
const Campaign=()=>{
    ...
    const [time, setTime] = useState(data?.data?.offerStartDate);
        ...
        <div className='flex flex-col md:flex-row justify-evenly px-4'>
           {
            ...
            <div className=' border-gray-800 flex  justify-center'>
             {
                
            products?.map((product,i:number)=>{
             
                     <div key={i} className='flex p-4 border border-dashed mb-3 rounded-md border-gray-800'>
                <img src={product?.productId?.image} className='w-32 h-fit' alt="" />
                <div className='ml-4 px-6'>
                    <p className='text-lg text-white font-semibold font-mono'>{product?.productId?.title}</p>
                    <p className='text-primary font-mono font-semibold'>Price:{product?.productId?.price}$</p>
